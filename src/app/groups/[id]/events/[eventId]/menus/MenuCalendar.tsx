...
export default function MenuCalendar({ 
  ...
  const getMenu = (date: Date, type: MealType) => {
    return menus.find(menu => {
      ...
      return menuDate.getFullYear() === date.getFullYear() &&
  ...
  const getNumberOfPeople = (date: Date, type: 'lunch' | 'dinner') => {
    return subgroups.reduce((total, subgroup) => {
      ...
      return total + (presence ? (type === 'lunch' ? presence.lunchNumber : presence.dinnerNumber) : 0);
    }, 0);
  };
    ...
    <div className="space-y-4 bg-gray-50 rounded-lg shadow-sm border p-6 mt-8">
      ...
      <div className="grid grid-cols-[auto,1fr]">
        <div>
          ...
          <div className="flex justify-between">
            ...
            <div>
              <div className={cn("h-24 px-2 border flex items-center justify-center bg-white text-gray-700")}>
                Déjeuner
              </div>
              <div className={cn("h-24 px-2 border flex items-center justify-center bg-white text-gray-700")}>
                Dîner
              ...
              <>
                ...
                <div 
                  ...
                  className={cn(
                    ...
                    getMenu(day, 'lunch') ? 'bg-green-100 hover:bg-green-200' : 'bg-gray-100',
                    getNumberOfPeople(day, 'lunch') > 0 && !getMenu(day, 'lunch') ? 'hover:bg-gray-200 cursor-pointer' : '',
                  )}
                  onClick={() => getNumberOfPeople(day, 'lunch') > 0 ? handleCellClick(day, 'lunch') : undefined}
                >
                  {getMenu(day, 'lunch') ? (
                    <>
                      <div className="flex flex-col justify-center">
                        <span className="font-medium text-gray-600">{getMenu(day, 'lunch')?.name}</span>
                        <span className="text-xs text-gray-500">{getMenu(day, 'lunch')?.numberOfPeople} personnes</span>
                        <span className="text-xs text-gray-500 mt-2">{getMenu(day, 'lunch')?.user ? getActiveAdultsNames(getMenu(day, 'lunch')!.user.id) : 'Inconnu'}</span>
                      </div>
                      {(getMenu(day, 'lunch')?.recipe || getMenu(day, 'lunch')?.url) && (
                        <div className="flex-shrink-0">
                          {getMenu(day, 'lunch')?.recipe ? (
                            <Link 
                              href={`/groups/${params.id}/recipes/${getMenu(day, 'lunch')?.recipe?.id}`}
                ...
                <div 
                  ...
                  className={cn(
                    "h-24 border flex items-center justify-between p-1 text-sm",
                    getMenu(day, 'dinner') ? 'bg-green-100 hover:bg-green-200' : 'bg-gray-100',
                    getNumberOfPeople(day, 'dinner') > 0 && !getMenu(day, 'dinner') ? 'hover:bg-gray-200 cursor-pointer' : '',
                  )}
                  onClick={() => getNumberOfPeople(day, 'dinner') > 0 ? handleCellClick(day, 'dinner') : undefined}
                >
                  {getMenu(day, 'dinner') ? (
                    <>
                      <div className="flex flex-col justify-center">
                        <span className="font-medium text-gray-600">{getMenu(day, 'dinner')?.name}</span>
                        <span className="text-xs text-gray-500">{getMenu(day, 'dinner')?.numberOfPeople} personnes</span>
                        <span className="text-xs text-gray-500 mt-2">{getMenu(day, 'dinner')?.user ? getActiveAdultsNames(getMenu(day, 'dinner')!.user.id) : 'Inconnu'}</span>
                      </div>
                      {(getMenu(day, 'dinner')?.recipe || getMenu(day, 'dinner')?.url) && (
                        <div className="flex-shrink-0">
                          {getMenu(day, 'dinner')?.recipe ? (
                            <Link 
                              href={`/groups/${params.id}/recipes/${getMenu(day, 'dinner')?.recipe?.id}`}
                              className="text-xs text-gray-600 hover:text-gray-900"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <ExternalLink className="h-3 w-3" />
      ...
      {selectedDate && selectedMealType && (
        <AddMealModal
          ...
          recipes={recipes}
          numberOfPeople={getNumberOfPeople(selectedDate, selectedMealType)}
          onAdd={() => {
